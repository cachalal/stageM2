...
grouped_data_2 <- data_set %>%
  group_by(ID, xCMI, TIME, CONC_TMP, Idbis) %>%
...
areas <- grouped_data_2 %>%
  group_by(ID, Idbis, xCMI, CONC_TMP) %>%
...
essai_model_lm <- nlsLM(
 delta_i_x ~ delta_i_max - ((delta_i_max - delta_i_min) * ((CONC_TMP / EC2LOG)^gam) / (((-2 - delta_i_min) / delta_i_max + 2) + (CONC_TMP / EC2LOG)^gam)),
  data = tableau_delta_i_x_clean,
  start = start_values)


#obtenir les valeurs des parametres
summary(essai_model_lm)

#obtenir la valeur de R?
rsquare(essai_model_lm,tableau_delta_i_x_clean)
...
tableau_delta_i_x %>% ggplot()+ geom_point(aes(x = AUC_CMI, y =delta_LOG10)) +   
