...
Ix <- ifelse(areas$ID %in% c(3, 5, 33), Ix / 8, Ix)
...
tableau_delta_i_x <- data.frame(ID = areas$ID,
                                xCMI = areas$xCMI,
                                delta_i_x = delta_i_x,
                                CONC = areas$CONC)  # Ajouter la colonne CONC
...
auc_test <- grouped_data_LR %>%
  filter(CONC != 0) %>%
  group_by(ID, ratio, CONC,xCMI) %>%
  summarize(AUC_test = calculate_area_trapezoids_LR(TIME, avg_LOG_CFU), .groups = 'drop')
...
auc_combined <- auc_combined %>%
  filter(!is.na(AUCFU_ref) & !is.na(AUC_test))

# Calculer LR
auc_combined <- auc_combined %>%
  mutate(LR = AUCFU_ref / AUC_test)

# Selectionner uniquement les colonnes necessaires pour le tableau final
